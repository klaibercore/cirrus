 ...
 *
 * Mirrors [dev.klaiber.cirrus.data.remote.ApiCredentials]: the token is persisted encrypted in
 * DataStore behind suspending reads, but an interceptor runs on a blocking thread and cannot
 * suspend, so a coroutine keeps this volatile copy in step.
 */
@Singleton
class GitHubCredentials @Inject constructor() {
     ...
     * server, which is why it is here rather than being an injected constructor parameter that
     * Dagger would have to bind a bare String for.
     */
    @Volatile
