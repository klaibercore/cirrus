 ...
 *
 * "It is disabled" is not actionable; "Settings → Tools → Apps and media" is — but only while that
 * row exists. A path that has drifted from the interface is worse than no path at all, because it
 * sends somebody looking for something that is not there and then makes them doubt the rest of the
...
class SettingsCatalogTest {
    ...
    fun `a switch that is off reports off, and says how to turn it on`() {
        val settings = AppSettings(locationEnabled = false)

        assertEquals("off", SettingSwitch.LOCATION.status(settings))
        val remedy = SettingSwitch.LOCATION.remedy(settings)
        assertNotNull(remedy)
        assertTrue(
            "the remedy has to carry the path, since that is the whole point of it",
            remedy!!.contains(SettingSwitch.LOCATION.path),
        )
    }
    ...
    fun `the defaults match what the switches claim about them`() {
        ...
        assertTrue("location is personal, so it starts off", !SettingSwitch.LOCATION.isOn(defaults))
