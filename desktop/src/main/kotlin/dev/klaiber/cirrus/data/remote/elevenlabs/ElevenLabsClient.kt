 ...
 *
 * Speaks HTTP and nothing else: it knows how to turn a string into an audio file and how to list
 * the voices on the account, and nothing about messages, markdown or playback. The audio is written
 * straight to a file rather than held in memory because a long answer is a few megabytes and the
 * player reads it as a stream anyway.
 */
@Singleton
class ElevenLabsClient @Inject constructor(
    // Its own client, for the reason GitHub and Spotify have theirs: the Ollama client
    // attaches the Ollama key to every request, and it must never reach a third party.
    ...
    companion object {
        ...
        /**
         * Raw 24 kHz PCM rather than the Android build's MP3.
         *
         * The JVM's own `javax.sound.sampled` decodes no MP3 — playing one means shipping a
         * decoder — but it plays raw PCM directly given the format, which is known here and fixed.
         * 24 kHz because `pcm_44100` is gated behind ElevenLabs' Pro tier and the difference is
         * inaudible for speech.
         */
        private const val OUTPUT_FORMAT = "pcm_24000"

        /** What [OUTPUT_FORMAT] actually is, for the player: 24 kHz, 16-bit, mono, little-endian. */
        const val SAMPLE_RATE = 24_000f
