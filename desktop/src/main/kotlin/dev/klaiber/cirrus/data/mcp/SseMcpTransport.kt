...
class SseMcpTransport @Inject constructor(
    // Must be the credential-free client: an interceptor's `header()` replaces whatever the
    // request already carried, so a shared one would send every attached server somebody
    // else's token instead of its own. `AppContainer` is where that is guaranteed.
