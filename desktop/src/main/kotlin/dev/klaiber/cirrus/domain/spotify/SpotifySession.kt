...
class SpotifySession @Inject constructor(
    ...
    private val redirects: SpotifyRedirectListener,
) {
    ...
    /**
     * The whole browser trip, start to finish, as one call.
     *
     * Android splits this in two because the return leg arrives as an intent in a different part of
     * the app entirely, and the two halves can be separated by the process dying. Here the listener
     * is in this process and the trip is one suspend function, so the screen awaits a result rather
     * than arranging to be called back.
     */
    suspend fun signIn(): Result<String> {
        val start = beginSignIn()
        return when (val redirect = redirects.awaitRedirect(start.url)) {
            is SpotifyRedirect.Code -> completeSignIn(redirect.code, redirect.state)
            is SpotifyRedirect.Denied -> {
                settings.consumeSpotifyPendingAuth()
                Result.failure(SpotifyException.Auth(redirect.reason))
            }

            is SpotifyRedirect.Failed -> {
                settings.consumeSpotifyPendingAuth()
                Result.failure(SpotifyException.Auth(redirect.reason))
            }
        }
     ...
     *
     * The verifier is held in memory rather than persisted: the listener that will need it is in
     * this same process, so it cannot outlive it usefully. Android persists it because the OS may
     * kill Cirrus while somebody reads the consent screen.
     */
    private suspend fun beginSignIn(): SpotifySignIn {
     ...
     *
     * The state check is not ceremony: without it, anything able to reach the loopback port could
     * hand Cirrus an authorization code of somebody else's choosing, and Cirrus would dutifully
     * exchange it and connect the user's app to an account they have never seen.
     */
    private suspend fun completeSignIn(code: String, state: String): Result<String> = runCatching {
